const { batchedDbUpdate, moveDataAfterInitialSync } = require('./util');
const {
...
async function onFinishInitialIngest(lib,data) {
  const { muAuthSudo } = lib;

  console.log(`!! On-finish triggered !!`);

  await moveDataAfterInitialSync(muAuthSudo.updateSudo, endpoint)
}
