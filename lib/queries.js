...
export async function getKboIdentifiers(administrativeUnit, changeTime) {
  const queryStr = `
...
export async function createNewKboOrg(kboOrg, kboId, isUpdate, oldKbo){
  ...
  INSERT {
    GRAPH ?g {
      ${sparqlEscapeUri(adressUri)} a <http://www.w3.org/ns/locn#Address> ;
                                      <http://mu.semte.ch/vocabularies/core/uuid> ${sparqlEscapeString(adressUuidId)} ;
                                      <http://www.w3.org/ns/locn#fullAddress> ${sparqlEscapeString(kboOrg.formattedAddress)} .
      ...
      ${sparqlEscapeUri(newKboIdUri)} a <http://mu.semte.ch/vocabularies/ext/KboEenheid> ;
                                        ...
                                        <http://mu.semte.ch/vocabularies/ext/shortName> ${sparqlEscapeString(kboOrg.shortName)} ;
                                        <http://mu.semte.ch/vocabularies/ext/startDate> ${sparqlEscapeString(kboOrg.startDate)} ;
                                        ...
                                        <http://www.w3.org/ns/adms#identifier> ${sparqlEscapeUri(kboId)} ;
                                        <http://www.w3.org/ns/regorg#orgStatus> ${sparqlEscapeUri(kboOrg.activeState)} ;
...
export async function linkAbbOrgToKboOrg(abbOrgUri ,kboOrgUri){
  ...
  INSERT {
    ...
    ${sparqlEscapeUri(kboOrgUri)} a <http://mu.semte.ch/vocabularies/ext/KboEenheid> . 
