  ...
  const queryStr = `
    SELECT DISTINCT ?kbo ?kboId ?ovo ?kboStructuredId ?ovoStructuredId ?bestuurseenheid WHERE {
      ...
      GRAPH ?g {
        ...
        OPTIONAL {
          ?bestuurseenheid <http://www.w3.org/ns/adms#identifier> ?ovoId .
...
export async function createNewKboOrg(kboOrg, kboId) {
  ...
  if (kboOrg.commercialName) {
    updateStr += `;  <http://www.w3.org/2004/02/skos/core#altLabel> ${sparqlEscapeString(
      kboOrg.commercialName
    )}`;
  }
...
export async function updateKboOrg(newKboInfo, identifiers) {
  //update address
  updateKboAddress(newKboInfo, identifiers);

  //update ContactPoint
  updateKboContact(newKboInfo, identifiers);

  //update kboUnit
  updateKboAdministrativeUnit(newKboInfo, identifiers);
}
...
async function updateKboAdministrativeUnit(newKboInfo, identifiers) {
  let updateStr = `
  DELETE{
    GRAPH ?g{
      ?kboUnit a <http://mu.semte.ch/vocabularies/ext/KboEenheid> ;
