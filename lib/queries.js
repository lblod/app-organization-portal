...
export async function getKboIdentifiers(administrativeUnit) {
  const queryStr = `
...
export async function getAllAbbKboOrganizations(){
   let queryStr = `
   SELECT ?kbo ?abbOrg ?kboOrg ?changeTime ?kboId where {
    GRAPH ?g {
      ?kboId <https://data.vlaanderen.be/ns/generiek#gestructureerdeIdentificator> ?kboStructuredId ;
            <http://www.w3.org/2004/02/skos/core#notation> ?kboNotation .
...
export async function createNewKboOrg(kboOrg, kboId) {
  ...
  console.log("creating:" + kboOrg);
  console.log("with:" + kboId);
  ...
  INSERT {
    GRAPH ?g {
      ...
      ${sparqlEscapeUri(contactPointUri)} a <http://schema.org/ContactPoint> ;
                                            ...
                                            <http://schema.org/email> ${sparqlEscapeString(
                                              kboOrg.email
                                            )} ;
                                            <http://schema.org/telephone> ${sparqlEscapeString(
                                              kboOrg.phone
                                            )} ;
                                            <http://schema.org/contactType> ${sparqlEscapeString(
                                              "Kbocontact"
                                            )} ;
                                            <http://xmlns.com/foaf/0.1/page> ${sparqlEscapeString(
                                              kboOrg.website
                                            )} ;
                                            <http://www.w3.org/ns/locn#address> ${sparqlEscapeUri(
                                              adressUri
                                            )}.
      ...
      ${sparqlEscapeUri(
        newKboIdUri
                                        ...
                                        <http://www.w3.org/ns/adms#identifier> ${sparqlEscapeUri(
                                          kboId
                                        )} ;
                                        <http://www.w3.org/ns/regorg#orgStatus> ${sparqlEscapeUri(
                                          kboOrg.activeState
                                        )} ;
                                        <http://schema.org/contactPoint> ${sparqlEscapeUri(
                                          contactPointUri
                                        )} .
    }
  }WHERE{
...
export async function updateKboOrg(newKboInfo, identifiers) {
  ...
  INSERT{
    graph ?g{ 
      ?adressUri a <http://www.w3.org/ns/locn#Address> ;
                <http://www.w3.org/ns/locn#fullAddress> ${sparqlEscapeString(newKboInfo.formattedAddress)} .
    }
  }
  WHERE{
    VALUES ?g {
